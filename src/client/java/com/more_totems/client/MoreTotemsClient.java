...
import com.more_totems.TotemEnchantPayload;
...
public class MoreTotemsClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(TotemNoFallPayload.TYPE, (payload, context) -> {
			Minecraft client = context.client();
			LocalPlayer player = client.player;
			if (player == null) return;

			client.gameRenderer.displayItemActivation(new ItemStack(ModItems.TOTEM_OF_NO_FALL));
		...
		ClientPlayNetworking.registerGlobalReceiver(TotemEnchantPayload.TYPE, (payload, context) -> {
			Minecraft client = context.client();
			LocalPlayer player = client.player;
			if (player == null) return;

			client.gameRenderer.displayItemActivation(new ItemStack(ModItems.TOTEM_OF_ENCHANT));

			// Enchant sparkles: cyan/enchantment-style particles
			RandomSource random = player.getRandom();
			for (int i = 0; i < 40; i++) {
				player.level().addParticle(
